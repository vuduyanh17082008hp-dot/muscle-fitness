...
const workoutPlanSchema = z.object({
  ...
  name: z
    ...
    .min(
      2,
      "Tên workout plan phải có ít nhất 2 ký tự.",
    )
    .max(
      120,
      "Tên workout plan không được vượt quá 120 ký tự.",
    ),
...
function normalizePlanInput(
  ...
  return {
    ...
    description: getNullableString(source, [
      "description",
      "notes",
    ]),
...
function normalizeDayInput(
  ...
  return {
    ...
    notes: getNullableString(source, [
      "notes",
      "description",
    ]),
...
export async function createWorkoutPlanAction(
  ...
  const parsedPlan = workoutPlanSchema.safeParse(
  ...
  if (!parsedPlan.success) {
    return {
      ...
      fieldErrors:
        parsedPlan.error.flatten().fieldErrors,
    };
  }
  ...
  try {
    ...
    const {
      ...
      .insert({
        ...
        goal: emptyToNull(
          parsedPlan.data.goal,
        ),
        weeks: parsedPlan.data.weeks,
        days_per_week:
          parsedPlan.data.daysPerWeek,
        session_duration_minutes:
          parsedPlan.data
            .sessionDurationMinutes,
        status: "draft",
    ...
    if (
      ...
      return {
        ...
        message: getDatabaseMessage(
          ...
          planError?.message ??
            "Không nhận được workout plan ID.",
        ),
      };
    }
    ...
    for (
      let dayIndex = 0;
      dayIndex <
      parsedPlan.data.days.length;
      dayIndex += 1
    ) {
      const parsedDay =
        workoutDaySchema.safeParse(
          normalizeDayInput(
            parsedPlan.data.days[dayIndex],
            dayIndex + 1,
          ),
        );
      ...
      const {
        ...
        .insert({
          ...
          day_number:
            parsedDay.data.dayNumber,
          name: parsedDay.data.name,
          focus: emptyToNull(
            parsedDay.data.focus,
          ),
          notes: emptyToNull(
            parsedDay.data.notes,
          ),
          rest_day:
            parsedDay.data.restDay,
        })
        .select("id")
        .single();
      ...
      if (
        ...
        return {
          ...
          message: getDatabaseMessage(
            ...
            dayError?.message ??
              "Không nhận được workout day ID.",
          ),
        };
      }
      ...
      for (
        ...
        const {
          ...
          .insert({
            ...
            rep_min:
              parsedExercise.data.repMin,

            rep_max:
              parsedExercise.data.repMax,
            ...
            tempo: emptyToNull(
              parsedExercise.data.tempo,
            ),

            rir:
              parsedExercise.data.rir ??
              null,

            notes: emptyToNull(
              parsedExercise.data.notes,
            ),
          });
    ...
    return {
      success: true,
      message:
        "Workout plan đã được tạo thành công.",
      planId,
    };
  } catch (error) {
    console.error(
      "createWorkoutPlanAction failed:",
      error,
    );
...
export async function createWorkoutDayAction(
  ...
  const planId = getFormString(
    formData,
    "workout_plan_id",
  );

  const parsedPlanId =
    uuidSchema.safeParse(planId);
...
export async function createWorkoutExerciseAction(
  ...
  const parsedExercise =
    workoutExerciseSchema.safeParse({
      ...
      exerciseName: getFormString(
        formData,
        "exercise_name",
      ),

      exerciseOrder: getFormString(
        formData,
        "exercise_order",
      ),

      targetSets: getFormString(
        formData,
        "target_sets",
      ),

      repMin: getFormString(
        formData,
        "rep_min",
      ),

      repMax: getFormString(
        formData,
        "rep_max",
      ),

      restSeconds: getFormString(
        formData,
        "rest_seconds",
      ),
...
export async function deleteWorkoutDayAction(
  ...
  if (error) {
    throw new Error(
      `Không thể xóa workout day: ${error.message}`,
    );
  }
...
function findSessionId(
   ...
   */
  for (
    const argument of actionArguments
  ) {
...
async function callWorkoutSessionApi(
  ...
  try {
    const {
      ...
      "@/lib/workouts/session-mutations"
    );
    ...
    console.error(
      `Workout action ${actionName} failed:`,
      error,
    );
...
export async function finishWorkoutAction(
  ...
  if (!sessionId) {
    return {
      success: false,
      message:
        "Không tìm thấy workout session ID.",
    };
  }
